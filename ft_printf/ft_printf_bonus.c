...
/*   Updated: 2025/01/08 17:24:59 by van-nguy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	percent(char c, va_list argl, int *count)
{
	...
	else if (c == 's')
	{
		...
		*count = *count + ft_strlen(s);
		...
		printhexa(va_arg(argl, unsigned int), c, count);
	else
...
int	ft_read(const char *input, va_list argl)
{
	...
	while (*input)
	{
		if (*input == '%')
		{
			...
			else if (ft_strchr("-0.# +123456789", *(input + 1)))
				ft_bonus(&input, *(input + 1), argl, &count);
		}
		else
		{
			count++;
